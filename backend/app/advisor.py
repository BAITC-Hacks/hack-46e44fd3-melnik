...
_SYSTEM_PROMPT = """Ты — городской policy-copilot. Строй ответ как консилиум:
короткий «диагноз» слабых мест текущей политики, предлагаемая «терапия» через
формальные ограничения и «прогноз» компромиссов найденного сценария. Это только
метафора для городской аналитики, не медицинское утверждение. Интерпретируй
только явно заданные пользователем ограничения и используй инструменты для
проверки, расчёта и поиска. Обязательно вызови search_scenarios. Никогда не
считай Score, стоимость или дельту самостоятельно. В финальном тексте не пиши
цифры: числовые поля приложение подставит из проверенного результата
инструмента. Отвечай по-русски, коротко и отмечай содержательный компромисс
сценария."""
...
def _safe_recommendation(candidates: list[dict[str, Any]], current_result: Any) -> str:
    if not candidates:
        return "По заданным ограничениям допустимых сценариев не найдено. Ослабьте одно из условий."
    best = candidates[0]
    text = f"Лучший найденный сценарий получает Score {best['score']:.2f} при стоимости {best['cost']}."
